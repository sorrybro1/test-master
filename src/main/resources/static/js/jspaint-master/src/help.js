...
function show_help(){
	...
	$iframe.attr({name: "help-frame", src: "../js/jspaint-master/help/default.html"});
	...
	$default_item_li.append($Item("Welcome to Help").click(function(e){
		$iframe.attr({src: "../js/jspaint-master/help/default.html"});
	}));
	$contents.append($default_item_li);
	
	$.get("../js/jspaint-master/help/mspaint.hhc", function(hhc){
		$($.parseHTML(hhc)).filter("ul").children().each(function(i, li){
			...
			$(li).children("ul").children().each(function(i, li){
				...
				$item_li.append($Item(object.Name).click(function(e){
					$iframe.attr({src: "../js/jspaint-master/help/" + object.Local});
				}));
